...
Paste the output ID into `wrangler.toml`, commit, and push.
...
- **Worker-level rate limiting** — Cloudflare Workers Rate Limiting bindings throttle `/mcp` (60 req/min per bearer token) and the OAuth flow endpoints (10 req/min per IP). Excess returns HTTP 429. Configured in `wrangler.toml` (`[[ratelimits]]`).
